        ...
        for d in range(list_len):
            ...
            for LR_key in order:
                ...
                if ini == 'L':
                    ...
                    # 直交する辺の座標ペア
                    if num == 0:
                        choku_cords = [cord2[num], cord2[new_nodes - 1]]
                    else:
                    ...
                    # 内積を計算して交差する角度を求める
                    # ベクトルA　交点とL1点を結ぶベクトル
                    f_vector_length(choku_cords[0][1], choku_cords[0][0], int_1st_x, int_1st_y)
                    vect_ax = vect_x
                    vect_ay = vect_y
                    vector_a = vector
                    # ベクトルB　交点と２つ目の点を結ぶベクトル
                    f_vector_length(taiko_cords[0][1], taiko_cords[0][0], int_1st_x, int_1st_y)
                    vect_bx = vect_x
                    vect_by = vect_y
                    vector_b = vector
            ...
            if new_nodes == 6:
                ...
                # 直交する辺の座標ペア
                if num == (new_nodes - 1):
                    choku_cords_2 = [cord2[num], cord2[0]]
                else:
                    choku_cords_2 = [cord2[num], cord2[num + 1]]

                # もう一方の対向する辺は，L点から３つ目と４つ目の点で結ばれる線分
                # 対向する辺-2の座標ペア
                taiko_cords_2 = []
                if (num + 3) > (new_nodes - 1):
                    taiko_cords_2.append(cord2[num + 3 - new_nodes])
                else:
                    taiko_cords_2.append(cord2[num + 3])
                if (num + 4) > (new_nodes - 1):
                    taiko_cords_2.append(cord2[num + 3 - new_nodes + 1])
                else:
                    taiko_cords_2.append(cord2[num + 4])
                print("taiko_cords_2", taiko_cords_2)

                # 直交する辺の両端座標（一方がL点）
                x1 = choku_cords_2[0][1]
                y1 = choku_cords_2[0][0]
                x2 = choku_cords_2[1][1]
                y2 = choku_cords_2[1][0]
                # 直交する直線の方程式
                line_1 = f_make_line(x1, y1, x2, y2)
                print(line_1)
                m1 = line_1['m']
                n1 = line_1['n']
                # 対向する辺-2の両端座標
                x1 = taiko_cords_2[0][1]
                y1 = taiko_cords_2[0][0]
                x2 = taiko_cords_2[1][1]
                y2 = taiko_cords_2[1][0]
                # 対向する直線-2の方程式
                line_2 = f_make_line(x1, y1, x2, y2)
                print(line_2)
                m2 = line_2['m']
                ...
                # 内積を計算して交差する角度を求める
                # ベクトルA　交点とL1点を結ぶベクトル
                f_vector_length(choku_cords[0][1], choku_cords[0][0], int_2nd_x, int_2nd_y)
                vect_ax = vect_x
                vect_ay = vect_y
                vector_a = vector
                # ベクトルB　交点と３つ目の点を結ぶベクトル
                f_vector_length(taiko_cords_2[0][1], taiko_cords_2[0][0], int_2nd_x, int_2nd_y)
                vect_bx = vect_x
                vect_by = vect_y
                vector_b = vector
                ...
                # print(int_1st_y)
                # 交点１までの距離
                div_line_a = math.sqrt((cord2[num][1] - int_1st_x) ** 2 + (cord2[num][0] - int_1st_y) ** 2)
                print("div_line_a=", div_line_a)
                # 交点２の座標
                # print(int_2nd_x)
                # print(int_2nd_y)
                # 交点２までの距離
                div_line_b = math.sqrt((cord2[num][1] - int_2nd_x) ** 2 + (cord2[num][0] - int_2nd_y) ** 2)
