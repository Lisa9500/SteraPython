    ...
    def build_Start(self):
        ...
        def flat_Vert(new_nodes):
            ...
            for j in range(new_nodes):
                ...
                if (j != 0):
                    ...
                    vect_bx = cord2[new_nodes - 1][1] - cord2[0][1]
                    vect_by = cord2[new_nodes - 1][0] - cord2[0][0]
                    vector_b = math.sqrt(vect_bx ** 2 + vect_by ** 2)
                    print(j, "ベクトルBx", vect_bx)
                    print(j, "ベクトルBy", vect_by)
                    print(j, "ベクトルB", vector_b)
        ...
        def ccw_Chk(new_nodes):
            ...
            for j in range(new_nodes):
                ...
                ys = cord2[j][0]
                if (j == 0):
                    xp = cord2[new_nodes - 1][1]
                    yp = cord2[new_nodes - 1][0]
                    xn = cord2[j + 1][1]
                    yn = cord2[j + 1][0]
                elif (j == new_nodes - 1):
                    xp = cord2[j - 1][1]
        ...
        def back_Reverse(new_nodes):
            rev_codes = copy.deepcopy(cord2)
            for j in range(new_nodes - 1, 0):
           ...
           # 頂点が時計回りの場合は，反時計回りに変更する
            ...
            for LR_key in order:
                ...
                if (ini == 'L'):
                    ...
                    # 直交する辺の座標ペア
                    choku_cords = []
                    choku_cords.append(cord2[num])
                    choku_cords.append(cord2[num - 1])
                    ...
                    # 内積を計算して交差する角度を求める
                    # ベクトルA　交点とL1点を結ぶベクトル
                    vector_Length(choku_cords[0][1], choku_cords[0][0], int_X, int_Y)
                    vect_ax = vect_x
                    vect_ay = vect_y
                    vector_a = vector

                    # ベクトルB　交点と２つ目の点を結ぶベクトル
                    vector_Length(taiko_cords[0][1], taiko_cords[0][0], int_X, int_Y)
                    vect_bx = vect_x
                    vect_by = vect_y
                    vector_b = vector
